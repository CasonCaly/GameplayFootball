    ...
    controllers(controllers)
{
    ...
    boost::intrusive_ptr<Resource<Surface>> kit = ResourceManagerPool::GetInstance().GetManager<
        Surface>(e_ResourceType_Surface)->Fetch(kitFilename);
    ...
    boost::intrusive_ptr<Resource<SoundBuffer>> soundBufferRes = ResourceManagerPool::GetInstance().GetManager<
        SoundBuffer>(e_ResourceType_SoundBuffer)->Fetch("media/sounds/crowd01.wav", true, true);
    ...
    LoadingMatchPage* loadingMatchPage = static_cast<LoadingMatchPage*>(menuTask->GetWindowManager()->GetPageFactory()->
...
void Match::SetRandomSunParams()
{
    ...
    sunPos.Normalize();
    if (random(0, 1) > 0.5f && sunPos.coords[1] > 0.25f) sunPos.coords[1] = -sunPos.coords[1];
    // sun more often on (default) camera side (coming from front == clearer lighting on players)
...
void Match::UpdateIngameCamera()
{
    ...
    Vector3 ballPos = ball->Predict(0) * (1.0f - playerBias) + GetDesignatedPossessionPlayer()->GetPosition() *
        playerBias;
    ...
    ballPos += Vector3(
        ((teams[0]->GetFadingTeamPossessionAmount() - 1.0f) * -teams[0]->GetSide() + (teams[1]->
            GetFadingTeamPossessionAmount() - 1.0f) * -teams[1]->GetSide()) * 4.0f, 0, 0);
    ...
    {
        float weight = sin((index / indexSize - 0.3f) * 1.4f * pi) * 0.5f + 0.5f;
        // healthy mix of latest & middle | wa: sin((x / 100 - 0.3) * 1.4 * pi) * 0.5 + 0.5 | from x = 0 to 100
        weight *= pow(1.0f - index / indexSize, 0.3f);
        // sharp cutoff @ latest (because cameraperson can't 'foresee' the current moment that fast) | wa: (1.0 - x / 100) ^ 0.3 * (<prev formula>) | from x = 0 to 100
    ...
    if (!IsGoalScored() || (IsGoalScored() && goalScoredTimer < 1000))
    {
        if (camMethod == 1)
        {
            ...
            cameraNodePosition = average * Vector3(
                1.0f * (1.0f - cameraUserAngleFactor * 0.2f) * (1.0f - cameraUserZoom * 0.3f),
                0.9f - cameraUserZoom * 0.3f, 0.2f) + Vector3(
                0, -41.4f - (cameraUserFOV * 3.7f) + pow(height, 1.2f) * 0.46f, 10.0f + height) * zoom;
...
void Match::Process()
{
    ...
    if (!pause)
    {
        ...
        if (GetBestPossessionTeamID() != -1)
        {
            ...
            if (!IsGoalScored())
            {
                if (IsInPlay())
                {
                    float distance = (Vector3(pitchHalfW * -teams[GetBestPossessionTeamID()]->GetSide(), 0, 0) -
                            GetPlayer(teams[GetBestPossessionTeamID()]->GetBestPossessionPlayerID())->GetPosition()).
                        GetLength();
            ...
            crowd02->SetGain(
                clamp((excitement - 0.3f) * 1.43f, 0.0f, 1.0f) * 0.5f * GetConfiguration()->GetReal(
                    "audio_volume", 0.5f));
        }
        ...
        actualTime_ms += 10;
        if (IsGoalScored()) goalScoredTimer += 10;
        else goalScoredTimer = 0;

        if (IsInPlay() && !IsInSetPiece()) GetMatchData()->AddPossessionTime_10ms(
        ...
        if (IsInPlay())
        {
            ...
            if (t1goal || t2goal)
            {
                ...
                if (GetLastTouchTeamID(e_TouchType_Intentional_Kicked) == GetLastGoalTeamID() ||
                    GetLastTouchTeamID(e_TouchType_Intentional_Nonkicked) == GetLastGoalTeamID()) ownGoal = false;
                ...
                if (!ownGoal)
                {
                    ...
                    {
                        SpamMessage(
                            "GOAL for " + matchData->GetTeamData(GetLastGoalTeamID())->GetName() + "! " + lastGoalScorer
                            ->GetPlayerData()->GetLastName() + " scores!", 4000);
                    }
                    else
                    {
            ...
            GetReferee()->GetBuffer().stopTime < GetActualTime_ms() - 1000)
        {
            if (GetReferee()->GetBuffer().prepareTime > GetActualTime_ms())
            {
                ...
                cameraNearCap = 1;
                cameraFarCap = 220;
                if (officials->GetReferee()->GetCurrentFunctionType() == e_FunctionType_Special) referee->
                    AlterSetPiecePrepareTime(GetActualTime_ms() + 1000);
            }
            else
            {
                // back to normal
...
bool Match::CheckForGoal(signed int side)
{
    ...
    if (fabs(previousBallPos.coords[1]) > 3.7 && fabs(previousBallPos.coords[0]) > pitchHalfW - lineHalfW - 0.11) return
        false;
...
void Match::CheckHumanoidCollisions()
{
    ...
    for (unsigned int i1 = 0; i1 < players.size(); i1++)
    {
        ...
        if (totalForce > 0.0f)
        {
            ...
            {
                const PlayerBounce& bounce = playerBounces.at(i1).at(i2);
                bounceVec += (bounce.opp->GetMovement() - players.at(i1)->GetMovement()) * bounce.force * (bounce.force
                    / totalForce);
                                   ...
                                   std::vector<PlayerBounce>& p2Bounce)
{
        ...
        distance < (bouncePlayerRadius + similarPlayerRadius) * 2.0f)
    {
        ...
        */
        ...
        if (distance < bouncePlayerRadius * 2.0f)
        {
            ...
            if (p1->TouchPending() && p1->GetCurrentFunctionType() == e_FunctionType_Interfere) bounceBias += 0.1f +
                0.4f * p1->GetStat("technical_standingtackle");
            if (p1->TouchPending() && p1->GetCurrentFunctionType() == e_FunctionType_Sliding) bounceBias += 0.1f + 0.4f
                * p1->GetStat("technical_slidingtackle");
            if (p2->TouchPending() && p2->GetCurrentFunctionType() == e_FunctionType_Interfere) bounceBias -= 0.1f +
                0.4f * p2->GetStat("technical_standingtackle");
            if (p2->TouchPending() && p2->GetCurrentFunctionType() == e_FunctionType_Sliding) bounceBias -= 0.1f + 0.4f
                * p2->GetStat("technical_slidingtackle");
                ...
                p2 == p2->GetTeam()->GetDesignatedTeamPossessionPlayer())
            {
                ...
                float ballDistanceDiffFactor = clamp(std::min(p2BallDistance, 1.2f) - std::min(p1BallDistance, 1.2f),
                                                     -0.6f, 0.6f) * 1.0f;
                // std::min is cap so difference won't matter if ball is far away (so only used in battles about the ball)
            ...
            Vector3 offset1 = (p1pos - p2pos).GetNormalized(0) * (bouncePlayerRadius - distance * 0.5f) * (1.0f -
                bounceBias0to1) * 2.0f;
            Vector3 offset2 = (p2pos - p1pos).GetNormalized(0) * (bouncePlayerRadius - distance * 0.5f) * bounceBias0to1
                * 2.0f;
        ...
        if (distance < bouncePlayerRadius * 2.0f)
        {
            ...
            if ((p1->GetDebug() || p2->GetDebug()) && verbose) printf("backfacing: %f - %f; ", p1sensitivity,
                                                                      p2sensitivity);
            ...
            if ((p1->GetDebug() || p2->GetDebug()) && verbose) printf("velocity: %f - %f; ", p1sensitivity,
                                                                      p2sensitivity);

            if (p1->HasBestPossession() == true) p1sensitivity += 1.0f;
            if (p2->HasBestPossession() == true) p2sensitivity += 1.0f;

            if ((p1->GetDebug() || p2->GetDebug()) && verbose) printf("haspossession: %f - %f; ", p1sensitivity,
                                                                      p2sensitivity);
            ...
            if ((p1->GetDebug() || p2->GetDebug()) && verbose) printf("balance: %f - %f; ", p1sensitivity,
                                                                      p2sensitivity);
            ...
            if ((p1->GetDebug() || p2->GetDebug()) && verbose) printf("decposoffset: %f - %f", p1sensitivity,
                                                                      p2sensitivity);

            // penetration
            float penetrationWeight = 6.0f;
            float penetration = ((p1->GetPosition() + p1->GetMovement() * 0.03f) - (p2->GetPosition() + p2->
                GetMovement() * 0.03f)).GetLength();
            //if (p1->GetDebug() || p2->GetDebug()) printf("penetration: %f\n", pow(1.0f - NormalizedClamp(penetration, 0.0f, bouncePlayerRadius * 2.0f), 0.4f));
            p1sensitivity += pow(1.0f - NormalizedClamp(penetration, 0.0f, bouncePlayerRadius * 2.0f), 0.4f) *
                penetrationWeight;
            p2sensitivity += pow(1.0f - NormalizedClamp(penetration, 0.0f, bouncePlayerRadius * 2.0f), 0.4f) *
                penetrationWeight;
            ...
            if (p1sensitivity > trip0threshold)
            {
                ...
                {
                    p1->TripMe(
                        (p1->GetMovement() * 0.1f + p2->GetMovement() * 0.06f + bounceVec * 1.0f).GetNormalized(
                            bounceVec), tripType);
            ...
            if (p2sensitivity > trip0threshold)
            {
                ...
                {
                    p2->TripMe(
                        (p2->GetMovement() * 0.1f + p1->GetMovement() * 0.06f - bounceVec * 1.0f).GetNormalized(
                            -bounceVec), tripType);
    ...
    int tackle = 0;
    if ((p1->GetCurrentFunctionType() == e_FunctionType_Sliding || p1->GetCurrentFunctionType() ==
        e_FunctionType_Interfere) && p1->GetFrameNum() > 5 && p1->GetFrameNum() < 28) tackle += 1;
    if ((p2->GetCurrentFunctionType() == e_FunctionType_Sliding || p2->GetCurrentFunctionType() ==
        e_FunctionType_Interfere) && p2->GetFrameNum() > 5 && p2->GetFrameNum() < 28) tackle += 2;
...
void Match::CheckBallCollisions()
{
    ...
    for (int i = 0; i < (signed int)players.size(); i++)
    {
        ...
        float lastTouchBias = player->GetLastTouchBias(touchTimeThreshold_ms);
        ...
        if (lastTouchBias <= 0.01f && oppLastTouchBias > 0.01f/* && ballTowardsPlayer*/)
        {
            ...
            e_FunctionType functionType = player->GetCurrentFunctionType();
            if (functionType == e_FunctionType_Movement || functionType == e_FunctionType_Trip || functionType == e_FunctionType_Sliding || functionType == e_FunctionType_Interfere || functionType == e_FunctionType_Deflect)
                collisionAnim = true;
            bool onlyWhenDirectionChangedUnexpectedly = false;
            if (functionType == e_FunctionType_Interfere || functionType == e_FunctionType_Deflect)
                onlyWhenDirectionChangedUnexpectedly = true;

            bool directionChangedUnexpectedly = false;
            if (onlyWhenDirectionChangedUnexpectedly)
            {
                float unexpectedDistance = (GetMentalImage(
                        player->GetController()->GetReactionTime_ms() + player->GetFrameNum() * 10)->
                    GetBallPrediction(1000) - GetBall()->Predict(1000)).GetLength();
                // mental image from when the anim began
            ...
            if (collisionAnim && !player->HasUniquePossession() && (onlyWhenDirectionChangedUnexpectedly == directionChangedUnexpectedly))
            {
                ...
                // fake a big AABB for more blocking fun, or a small one for less bouncy bounce
                if (!player->HasPossession())
                    boundingBoxSizeOffset += 0.03f;
                else
                    boundingBoxSizeOffset -= 0.03f;

                if (functionType == e_FunctionType_Sliding || functionType == e_FunctionType_Interfere)
                {
                ...
                if (((player->GetPosition() + Vector3(0, 0, 0.8f)) - ball->Predict(0)).GetLength() < 2.5f)
                {
                    // premature optimization is the root of all evil :D
                    player->GetHumanoidNode()->GetObjects(e_ObjectType_Geometry, objectList);
                        ...
                        if (objAABB.Intersects(ball->Predict(0), ballRadius))
                        {
                            ...
                            else
                            {
                                ...
                                bounceVec += (ball->Predict(0) - (*objIter)->GetDerivedPosition()).
                                    GetNormalized(Vector3(0)) * movementBias + player->GetMovement() * (1.0f -
                                        movementBias);
                                bounceCount++;
                                player->GetTeam()->SetLastTouchPlayer(player, e_TouchType_Accidental);
                                Vector3 aabbCenter;
                                objAABB.GetCenter(aabbCenter);
                                bias += (1.0f - clamp(
                                    ((ball->Predict(0) - aabbCenter).GetLength() - ballRadius) / objAABB.GetRadius(),
                                    0.0f, 1.0f)) * 0.9f + 0.1f;
                            }
                        }
    ...
    if (bias > 0.0f)
    {
        ...
        Vector3 fullCollisionVec = (bounceVec * 6.0f) + (bounceVec * currentMovement.GetLength() * 0.6f) + (
...
void Match::PrepareGoalNetting()
{
    ...
    std::vector<MaterializedTriangleMesh>& triangleMesh = boost::static_pointer_cast<
        Geometry>(goalsNode->GetObject("goals"))->GetGeometryData()->GetResource()->GetTriangleMeshesRef();
    ...
    for (unsigned int m = 0; m < triangleMesh.size(); m++)
    {
        ...
        {
            int goalID = -1;
            if (triangleMesh.at(m).vertices[i + 0] < -pitchHalfW - 0.06f) goalID = 0;
            // don't catch woodwork, only netting.. DIRTY HAXX
    ...
    if (ballTouchesNet)
    {
        ...
        for (unsigned int i = 0; i < nettingMeshes[sideID].size(); i++)
        {
            ...
            float influenceBias = pow(
                clamp((shortestDistance + 0.0001f) / (vertex.GetDistance(ball->GetBallGeom()->GetPosition()) + 0.0001f),
                      0.0f, 1.0f), 1.5f);
